...
function renderExams() {
    ...
    // Waktu saat ini untuk perhitungan status
    const now = new Date(); 
    ...
    container.innerHTML = paginatedData.map(ex => {
        ...
        <tr class="bg-white border-b border-slate-100 hover:bg-slate-50 transition-colors">
            ...
            <td class="p-4 text-center">
                <div class="text-sm font-bold text-slate-700 flex justify-center items-center gap-1.5">
                    <i data-lucide="calendar" class="w-3.5 h-3.5 text-slate-400"></i> ${ex.exam_date}
                </div>
                <div class="text-xs font-medium text-slate-500 mt-1 flex justify-center items-center gap-1.5">
                    <i data-lucide="clock" class="w-3 h-3 text-slate-400"></i> ${ex.start_time} - ${ex.end_time}
                </div>
            </td>
            <td class="p-4 text-center">
            ...
            <td class="p-4 text-center">
                <span class="px-3 py-1.5 text-[10px] font-bold rounded-full shadow-sm ${statusClass}">
                    ${statusText}
                </span>
            </td>
            <td class="p-4 text-center">
    ...
    // Me-render ikon lucide yang baru dimuat
    lucide.createIcons();
    updateBulkDeleteBtn();
}
